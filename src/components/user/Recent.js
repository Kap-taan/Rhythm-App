...
import { getDocs, collection, query, orderBy, limit, doc, updateDoc, increment } from "firebase/firestore";
...
const Recent = ({ type }) => {
    ...
    const getSongs = async () => {
        ...
        const data = await getDocs(query(collection(db, "songs"), orderBy("count", "desc"), limit(10)));
        let tempSongs = [];
        data.forEach(doc => {
